...
typedef struct os_thread_def  {
  ...
  struct OS_TCB          tcb;
  uint32_t               period;         ///< millisec of the period
  uint32_t               budget;         ///< millisec of the budget per period
  uint32_t               deadline;       ///< millisec of the relative deadline
} osThreadDef_t;
...
/// Define to quickly identify EDF threads
#define EDF_PRIORITY (osPriorityEDF - osPriorityIdle + 1)
...
/// Return an integer representing the thread ID of the current running thread.
/// \return thread ID for reference by other functions or 0 in case of error.
/// \note MUST REMAIN UNCHANGED: \b osThreadGetId shall be consistent in every CMSIS-RTOS.
uint32_t osThreadReference ();
...
/// Terminate the actual period and yield the CPU. The task is suspended until next period
/// \return correct if the thread supports budget management
osStatus osThreadEndPeriod (void);
